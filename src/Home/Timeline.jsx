    ...
    <div className="container mx-auto mt-20 max-w-7xl px-4">
      ...
      {filteredTimeline.length > 0 ? (
        filteredTimeline.map((friend, ind) => (
          <div className="flex gap-6 items-center bg-white shadow-sm m-3 p-5 rounded-xl border border-gray-50" key={ind}>
            <div className="text-2xl bg-gray-50 p-3 rounded-full">
              {icons[friend.type]}
            </div>
            <div>
              <p className="text-lg">
                <span className="font-bold">{friend.type}</span> with <span className="text-gray-700">{friend.name}</span>
              </p>
        ...
        <p className="text-center text-gray-400 mt-10">No interactions found for {filterType}.</p>
      )}
    </div>
  );
};
